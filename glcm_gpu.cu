...
__global__ void kernel(int *image, char *mask, int *threads_glcm, int *dev_angles)
{

    int ix = blockIdx.x;
    int iy = threadIdx.x;

    int idx = ix * blockDim.x + iy;
...
void atomic_glcm_gpu(int *image, char *mask, int *glcm)
{
    ...
    HANDLE_ERROR(cudaMalloc((void **)&dev_mask, sizeof(char) * size[0] * size[1] * size[2]));
    ...
    HANDLE_ERROR(cudaMemcpy(dev_mask, mask, sizeof(char) * size[0] * size[1] * size[2], cudaMemcpyHostToDevice));
...
void reduce_glcm_gpu(int *image, char *mask, int *glcm)
{
...
void glcm_cpu(int *image, char *mask, int *glcm)
{
...
void glcm_cpu_parallel(int *image,char*mask,int *glcm){
...
int main()
{
    ...
    mask = (char *)malloc(sizeof(char) * size[0] * size[1] * size[2]);
